...
// --- BẢO MẬT: CHỐNG NHÂN BẢN TRÌNH DUYỆT ---
...
function uiSelectWwMode(mode) {
    wwMode = mode;
    document.getElementById('btn-mode-basic').classList.remove('active');
...
function uiGoToStep2() {
    playSfx('click');
    document.getElementById('ww-step-1-mode').style.display = 'none';
    document.getElementById('ww-step-2-visibility').style.display = 'block';
    uiSelectVisibility('public'); 
}

function uiBackToStep1() {
    playSfx('click');
    document.getElementById('ww-step-2-visibility').style.display = 'none';
    document.getElementById('ww-step-1-mode').style.display = 'block';
}

function uiSelectVisibility(vis) {
    wwVisibility = vis;
    document.getElementById('btn-vis-public').classList.remove('active');
    document.getElementById('btn-vis-private').classList.remove('active');
    document.getElementById(`btn-vis-${vis}`).classList.add('active');
    document.getElementById('ww-private-pass-container').style.display = (vis === 'private') ? 'block' : 'none';
...
function executeCreateRoom() {
    ...
    database.ref('rooms').once('value', (snap) => {
        ...
        if (!customName) {
            let maxNum = 0;
            for(let r in rooms) {
                let name = rooms[r].roomName || "";
                if (name.startsWith("Phòng ")) {
                    let num = parseInt(name.replace("Phòng ", ""));
                    if (!isNaN(num) && num > maxNum) maxNum = num;
                }
            }
            customName = "Phòng " + (maxNum + 1);
        }
